...
@Slf4j
@Component("EthClient")
public class EthClient implements Client {

  private String ethUrlPrefix;

  private static final int RE_TRY_TIMES = 5;

  private SecureRandom rand = new SecureRandom();
  @Resource
  private EventUrlConfigService eventUrlConfigService;

  @Value("${rpc.eth.chainId:1}")
  private long chainId;

  @PostConstruct
  void init() {
    List<String> dbList = eventUrlConfigService.queryUrlConfig("eth");
    if (CollectionUtils.isNotEmpty(dbList)) {
      ethUrlPrefix = dbList.get(0);
    } else {
      ethUrlPrefix = "";
      log.error("ETH event url config table is null");
    }
  }

  @Override
  public String postRetry(String json) {
    if (StringUtils.isBlank(ethUrlPrefix)) {
      return "";
    }
  ...
  public String postReTryWithRetryTimes(String json, Integer retryTimes) {
    ...
    try {
      String postResult = HttpClientUtil.doPostJsonUnCatch(ethUrlPrefix, json);
      ...
      if (result.containsKey("error")) {
        ...
        log.warn("EthClient postReTry error! eth url: {}, post json: {}, error: {}",
            ethUrlPrefix, json, postResult);
      ...
      log.warn("EthClient postReTry error! eth url: {}, post json: {}, error: {}",
          ethUrlPrefix, json, stackTraceStr);
    }
    ...
    if (retryTimes == 0) {
      log.error("EthClient postReTry fail! post json : {}", json);
    }
  ...
  public EthSendTransaction sendRawTransactionWithRetry(
    ...
    for (int i = 0; i < CommonConstant.ETH_RETRY_TIMES; ++i) {
      try {
        Web3j web3j = Web3j.build(new HttpService(ethUrlPrefix));
        ...
        log.warn("Web3j sendRawTransactionWithRetry throw exception: {}",
            Throwables.getStackTraceAsString(e));
        Thread.currentThread().interrupt();
      }
      try {
    ...
    log.error("Web3j sendRawTransactionWithRetry failed for eth, urlPrefix:{}, rawTransaction:{}",
        ethUrlPrefix, rawTransaction);
    return null;
  }
  ...
  public EthEstimateGas ethEstimateGas(Transaction gasEstimateTx) {
    ...
    log.error("Web3j ethEstimateGas failed for eth, ethUrlPrefix:{}, rawTransaction:{}",
        ethUrlPrefix, gasEstimateTx.toString());
    return null;
  }
  ...
  private boolean updateUrlPrefix() {
    ...
    if (dbList.contains(ethUrlPrefix)) {
      int index = dbList.indexOf(ethUrlPrefix);
      if (index < dbList.size() - 1) {
        index++;
      } else {
        index = 0;
      }
      ethUrlPrefix = dbList.get(index);
    } else {
      ethUrlPrefix = dbList.get(0);
    }
    return true;
